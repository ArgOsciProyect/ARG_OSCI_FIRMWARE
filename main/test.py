...
parser = argparse.ArgumentParser(description='Modo de graficado')
parser.add_argument('--mode', choices=['time', 'samples'], default='time', help='Modo de graficado: time o samples')
    ...
    try:
        while True:
            ...
            if elapsed_time >= UPDATE_TIME:
                ...
                y = np.array(int_data)
                if args.mode == 'time':
                    x = np.arange(len(int_data)) * PERIOD  # Convertir a tiempo
                else:
                    x = np.arange(len(int_data))  # Usar muestras

                f = interp1d(x, y, kind='linear')
                x_new = np.linspace(0, len(int_data) * (PERIOD if args.mode == 'time' else 1) - (PERIOD if args.mode == 'time' else 1), num=len(int_data))  # Ajustar el rango de x_new
                y_new = f(x_new)

                plt.figure()
                plt.plot(x_new, y_new)
                plt.title(f'Últimas muestras recibidas en los últimos 10 segundos (Interpolación) - Modo: {args.mode}')
                plt.xlabel('Tiempo (s)' if args.mode == 'time' else 'Muestras')
                plt.ylabel('Valor')
                plt.show()
